...
function AnswerButton({ onClick, children, bg, color, submitted, label, style: extraStyle }) {
  return (
    <button onClick={onClick}
      ...
      style={{
        background: bg, color, borderColor: color, borderRadius: "12px", padding: "1.1rem 1.25rem",
...
function OptionButton({ onClick, children, active = false, disabled = false, style: extraStyle }) {
  return (
    <button onClick={onClick} {...glassPress()}
      className={`button button-outline button-round${active ? ' button-fill' : ''}`}
      style={{
        ...
        ...(active ? { background: T.accent, color: "#fff", borderColor: T.accent } : { background: T.surface2, color: T.text, borderColor: T.border }),
        ...extraStyle,
      }}>
      {children}
    </button>
...
function AppCard({ onClick, onContextMenu, children, style: extraStyle, cardRef }) {
  ...
  return (
    <div
      ...
      style={{
        ...card({ marginBottom: "0.6rem", padding: "1.35rem" }),
        cursor: "pointer",
        overflow: "hidden",
...
// Downloads a single set as a .json file — shared by the hamburger menu's "Save set as" and the
// set card's right-click/long-press "Save set as", so both go through the same, file-download
// method instead of the old copy/paste-a-JSON-blob modal.
function exportSetAsFile(set) {
  const json = JSON.stringify([set]);
  const blob = new Blob([json], { type: "application/json" });
  const url = URL.createObjectURL(blob);
  const a = document.createElement("a");
  a.href = url;
  a.download = (set.name || "set").replace(/[^a-z0-9]/gi, "-").toLowerCase() + ".json";
...
const HISTORY_SEEN_KEY = "az305_history_seen_count";
// Question snapshots are stored once per unique set-of-questions content and referenced from
// history sessions by hash, instead of being duplicated inside every single session — see
// hashQuestions() / resolveSessionQuestions() below.
const HISTORY_SNAPSHOTS_KEY = "az305_history_question_snapshots";
...
// ── History question-snapshot dedup ────────────────────────────────────────
// A session's full question content (text, options, hints, explanations — everything) only
// needs to be stored once per unique set of questions. Repeated attempts at an unchanged set
// share the same snapshot instead of each carrying their own full copy, which is what made
// the history file balloon in size. Sessions store a short `qHash` reference instead of the
// `questions` array; `resolveSessionQuestions` looks the real content back up when needed.
function hashQuestions(questions) {
  const str = JSON.stringify(questions || []);
  let h1 = 0x811c9dc5, h2 = 0x1000193; // two independent FNV-1a-style seeds, combined below
...
function loadQuestionSnapshots() { try { return JSON.parse(lzGet(HISTORY_SNAPSHOTS_KEY)) || {}; } catch { return {}; } }
function saveQuestionSnapshots(s) { lzSet(HISTORY_SNAPSHOTS_KEY, JSON.stringify(s)); }
// Backward-compatible: older sessions still carry their own embedded `questions` array
// directly (pre-dedup format) and are returned as-is; newer ones resolve via `qHash`.
function resolveSessionQuestions(session, snapshots) {
  if (session?.questions) return session.questions;
  if (session?.qHash && snapshots && snapshots[session.qHash]) return snapshots[session.qHash];
  return [];
}
// Strips the (potentially large) embedded `questions` array from a session, replacing it with
// a `qHash` reference, and returns the snapshot entry that needs to be registered alongside it.
function dedupeSessionQuestions(session) {
  if (!session.questions) return { session, hash: null };
  const hash = hashQuestions(session.questions);
  const { questions, ...rest } = session;
  return { session: { ...rest, qHash: hash }, hash, snapshot: questions };
}

// Mastery blends two signals into one score:
//  1. Accuracy — for each question you've attempted, average its attempts with recency
//     decay (older attempts on that question count exponentially less than newer ones), so
//     recent slip-ups on a previously-mastered question pull the score down instead of being
//     diluted by a long history of past-correct answers.
//  2. Coverage — that accuracy is scaled by the fraction of the set's questions you've ever
//     attempted, so a set where only 3 of 50 questions have been answered can't read 100%.
// `questions` is the current question list for the set (used for the coverage denominator);
// falls back to attempted-only coverage (1.0) if omitted.
function computeMastery(sessions, questions) {
  const qMap = {};
  sessions.forEach(s => {
    if (!s.results) return;
    s.results.forEach(r => {
      if (!qMap[r.qId]) qMap[r.qId] = [];
      qMap[r.qId].push({ correct: r.correct, date: s.date });
    });
  });

  const qIds = Object.keys(qMap);
  if (!qIds.length) return null;

  const ATTEMPT_DECAY = 0.85; // each older attempt of the same question counts 85% as much as the next-newer one
  const accuracies = qIds.map(qId => {
    const attempts = [...qMap[qId]].sort((a, b) => new Date(b.date) - new Date(a.date));
    let weightedCorrect = 0, weightedTotal = 0;
    attempts.forEach((a, i) => {
      const w = Math.pow(ATTEMPT_DECAY, i);
      weightedTotal += w;
  ...
  const attemptedAccuracy = accuracies.reduce((sum, a) => sum + a, 0) / accuracies.length;
  const total = questions && questions.length ? questions.length : accuracies.length;
  const coverage = Math.min(accuracies.length / total, 1);
...
const SET_ICONS = [
  { category: "Academic", icons: [
    ...
    { id: "math",          label: "Math",         path: "M14.3397 12h-1.075M16.1736 12h-1.075M18.0074 12h-1.075M19.8412 12h-1.075M10.7353 12h-1.075M8.9015 12h-1.075M7.0677 12h-1.075M5.2338 12h-1.075M12 7.8265v1.075M12 9.6603v1.075M12 13.2647v1.075M12 15.0986v1.075M12 16.9324v1.075M12 18.7662v1.075M12 5.9927v1.075M12 4.1588v1.075M5.1706 10.3559c10.8765 -11.0662 7.5882 6.45 13.7853 7.8412M11.625 12a0.375 0.375 0 1 0 0.75 0a0.375 0.375 0 1 0 -0.75 0" },
...
// Groups questions by topic and sorts the groups (untagged last, others alphabetically),
// returning topic + ordered id lists. Used to compute the question builder's display order —
// see stableGroupsRef in EditMode, which freezes this until the set is closed and reopened.
function buildTopicGroups(questions) {
  ...
  questions.forEach(q => {
    ...
    if (!(topic in groupByTopic)) {
      groupByTopic[topic] = { topic, ids: [] };
      groups.push(groupByTopic[topic]);
    }
    groupByTopic[topic].ids.push(q.id);
  });
  groups.sort((a, b) => {
...
function EditMode({ set, allTags, onSave, onBack, scrolled, onCanSaveChange, onQuestionCountChange, editSearch = "", isMobile = true }) {
  ...
  // Freezes the topic-grouped display order for this editing session — see the render logic
  // below where it's built/patched.
  const stableGroupsRef = useRef(null);
  ...
  return (
    <div style={{
      ...
      {(() => {
        // Build the grouping fresh only on first render of this editing session. After that,
        // patch it in place for structural changes only (questions added/removed) — never
        // resort or reassign an existing question's group just because a field (like topic)
        // changed mid-edit. The whole thing resets naturally when the set is closed and
        // reopened, since that mounts a fresh EditMode instance.
        if (!stableGroupsRef.current) {
          stableGroupsRef.current = buildTopicGroups(draft.questions);
        } else {
          const liveIds = new Set(draft.questions.map(q => q.id));
          const knownIds = new Set();
          stableGroupsRef.current.forEach(g => g.ids.forEach(id => knownIds.add(id)));
          // Drop deleted questions, and any group left empty as a result.
          stableGroupsRef.current = stableGroupsRef.current
            .map(g => ({ ...g, ids: g.ids.filter(id => liveIds.has(id)) }))
            .filter(g => g.ids.length > 0);
          // Newly-added questions join a matching existing group if one exists, or start a
          // brand-new group appended at the end — existing groups never move.
          draft.questions.forEach(q => {
            if (knownIds.has(q.id)) return;
            const topic = (q.topic || "").trim();
            const existing = stableGroupsRef.current.find(g => g.topic === topic);
            if (existing) existing.ids.push(q.id);
            else stableGroupsRef.current.push({ topic, ids: [q.id] });
          });
        }

        // Look up each question's current, live data by id (so text/option/topic edits show
        // immediately) while keeping the frozen order/grouping above.
        const byId = {};
        draft.questions.forEach((q, i) => { byId[q.id] = { q, i }; });
        const searchTerm = editSearch.trim().toLowerCase();

        const groups = stableGroupsRef.current
          .map(g => ({
            topic: g.topic,
            items: g.ids
              .map(id => byId[id])
              .filter(Boolean)
              .filter(({ q }) => !searchTerm || q.question?.toLowerCase().includes(searchTerm)),
          }))
...
function EditorFab({ onAddQuestion }) {
  ...
  return ReactDOM.createPortal(
    <>
      ...
      {/* Glassy Add button — stays a fixed circle; a separate label chip slides out to its
          left when idle, rather than the button itself growing into a pill. */}
      <div style={{ position: "fixed", bottom: "1.5rem", right: "1.5rem", zIndex: 100, display: "flex", alignItems: "center" }}>
        <span aria-hidden="true" style={{
          ...
          fontFamily: FF_SANS, fontSize: "0.85rem", fontWeight: 500, color: T.text,
          width: fabExpanded ? "128px" : "0px",
          marginRight: fabExpanded ? "0.6rem" : "0px",
          paddingTop: fabExpanded ? "0.35rem" : "0px",
          paddingBottom: fabExpanded ? "0.35rem" : "0px",
          paddingLeft: fabExpanded ? "0.65rem" : "0px",
          paddingRight: fabExpanded ? "0.65rem" : "0px",
          opacity: fabExpanded ? 1 : 0,
          transition: "width 0.3s ease, margin-right 0.3s ease, padding 0.3s ease, opacity " + (fabExpanded ? "0.2s ease 0.15s" : "0.1s ease"),
        }}>Add Question</span>
        <GradientBorderButton size={52} onClick={e => {
...
function ResultsScreen({ results, questions, set, onRestart, onBack, onSaveToHistory, questionLimit, examMode = false, isHistoryView, historyDate, onRetryMissed, exportModal, onCloseExport, confirmRetry, onCloseConfirmRetry, history, isMobile = true }) {
  ...
  const otherSetSessions = (history || [])
    .filter(h => {
  ...
  // Place the attempt currently on screen into its correct chronological position among every
  // other attempt for this set — rather than always drawing it as the most recent one. Opening
  // an older result from History should show earlier/later attempts on either side of it.
  const currentEntry = { __current: true, date: isHistoryView ? historyDate : session.date, score, total: results.length };
  const sortedSetSessions = [...otherSetSessions, currentEntry].sort((a, b) => new Date(a.date) - new Date(b.date));
  // Cap the trend at 6 entries, but keep the currently-viewed session inside that window even
  // when it isn't the most recent attempt — a plain trailing slice(-6) could otherwise drop it
  // off the front if 6+ newer attempts exist after it.
  const TREND_WINDOW = 6;
  const currentIdx = sortedSetSessions.findIndex(s => s.__current);
  let windowStart = Math.max(0, currentIdx - (TREND_WINDOW - 1));
  let windowEnd   = Math.min(sortedSetSessions.length, windowStart + TREND_WINDOW);
  windowStart = Math.max(0, windowEnd - TREND_WINDOW);
  const trendSessions = sortedSetSessions.slice(windowStart, windowEnd);

  // Open the trend row scrolled all the way to the right, since the most recent attempt
  // (chronologically) sits at that end.
      ...
      <div style={{ ...card({ marginBottom: "1.5rem", padding: 0, overflow: "hidden", borderColor: sColor + "55" }) }}>
        ...
        {trendSessions.length > 1 && (
          <div style={{ borderTop: "1px solid " + (T.mode === "light" ? "rgba(0,0,0,0.06)" : "rgba(255,255,255,0.06)"), padding: "1rem 1.25rem" }}>
            ...
            <div ref={trendScrollRef} className="no-scrollbar" style={{ display: "flex", gap: "0.5rem", alignItems: "flex-end", overflowX: "auto", scrollbarWidth: "none" }}>
              {trendSessions.map((s, i) => {
                const p   = s.__current ? pct : Math.round(s.score / s.total * 100);
                const col = s.__current ? sColor : scoreColor(p);
                return (
                  <Fragment key={s.__current ? "current" : (s.id || i)}>
                    {i > 0 && <TrendArrow />}
                    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: "0.25rem", flexShrink: 0 }}>
                      ...
                      <span style={{ fontFamily: FF_SANS, fontSize: "0.58rem", color: s.__current ? T.accent : T.muted,
                        fontWeight: s.__current ? 600 : 400, textAlign: "center", lineHeight: 1.2 }}>
                        {s.__current && !isHistoryView ? "today" : new Date(s.date).toLocaleDateString(undefined, { month: "short", day: "numeric" })}
                      </span>
                    </div>
                  </Fragment>
                );
...
function SessionPicker({ set, onStart, onClose, onEdit }) {
  ...
  // Shared between Review and Exam mode — defaults to the full set, matching the previous
  // behavior when nobody touched a count selector at all. Dropping below the full count is
  // what used to be "Quick Mode": same weighted-toward-weak-questions selection under the hood
  // (see questionLimit in ReviewMode / handleStart below), just reachable from either mode now
  // instead of being a separate menu entry.
  const [qCount,    setQCount]    = useState(count);
  const minCount = Math.min(5, count);

  const TIMER_OPTIONS = [30, 60, 90, 120];

  const stepLabel = {
    "mode":         "Study Session",
    "review-count": "Review Mode",
    "exam-timer":   "Exam Mode",
  }[step];

  const back = step === "review-count" || step === "exam-timer" ? () => setStep("mode") : null;
  // null when the count is left at the full set, so it behaves exactly like the old
  // unlimited Review/Exam mode (plain shuffle) instead of the weighted-subset picker.
  // Both qCount and customMin can be mid-edit (unclamped, possibly "") while the input is
  // focused — clamp here too rather than relying solely on the input's onBlur.
  const handleStart = (m, minutesRaw) => {
    const finalCount = Math.min(count, Math.max(minCount, parseInt(qCount, 10) || minCount));
    const finalMinutes = minutesRaw != null ? Math.min(240, Math.max(5, parseInt(minutesRaw, 10) || 5)) : null;
    onStart(finalCount === count ? null : finalCount, m, finalMinutes);
  };
      ...
      <ModalCard pad="1.75rem" maxWidth={420}>
        ...
        {step === "mode" && (
          <div style={{ display: "flex", flexDirection: "column", gap: "0.6rem" }}>
            <OptionButton onClick={() => { setMode("review"); setStep("review-count"); }}>
              <div style={{ display: "flex", alignItems: "center", gap: "0.65rem", width: "100%" }}>
                ...
                <div style={{ display: "flex", flexDirection: "column" }}>
                  ...
                  <span style={{ display: "block", fontFamily: FF_SANS, color: T.muted2, fontSize: "0.78rem", marginTop: "0.2rem", lineHeight: 1.3 }}>
                    Pick how many questions, answers as you go
                  </span>
                </div>
              </div>
            </OptionButton>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
            <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: "0.75rem",
              ...
              <div className="stepper stepper-large stepper-round stepper-raised"
                ...
                <div className="stepper-button-minus" onClick={() => setQCount(n => Math.max(minCount, n - 5))} />
                <div className="stepper-input-wrap">
                  <input type="text" inputMode="numeric" value={qCount}
                    onChange={e => { const raw = e.target.value.replace(/[^0-9]/g, ""); setQCount(raw === "" ? "" : parseInt(raw, 10)); }}
                    onBlur={() => setQCount(v => { const n = parseInt(v, 10); return Math.min(count, Math.max(minCount, isNaN(n) ? minCount : n)); })}
                    style={{ fontFamily: FF_SANS, textAlign: "center", color: T.text }} />
                </div>
                <div className="stepper-button-plus" onClick={() => setQCount(n => Math.min(count, n + 5))} />
              </div>
              <p style={{ fontFamily: FF_SANS, fontSize: "0.75rem", color: T.muted }}>
                {qCount === count ? `all ${count} questions` : `of ${count} questions`}
              </p>
            </div>

            <PrimaryButton onClick={() => handleStart("review")} style={{ width: "100%", justifyContent: "center", height: "52px", fontSize: "1rem" }}>
              Start
            </PrimaryButton>
          </div>
        )}

        {/* Step: exam timer + question count */}
        {step === "exam-timer" && (
          <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
              ...
              <div className="stepper stepper-large stepper-round stepper-raised"
                ...
                <div className="stepper-input-wrap">
                  <input type="text" inputMode="numeric" value={customMin}
                    onChange={e => { const raw = e.target.value.replace(/[^0-9]/g, ""); setCustomMin(raw === "" ? "" : parseInt(raw, 10)); }}
                    onBlur={() => setCustomMin(v => { const n = parseInt(v, 10); return Math.min(240, Math.max(5, isNaN(n) ? 5 : n)); })}
                    style={{ fontFamily: FF_SANS, textAlign: "center", color: T.text }} />
                </div>
            ...
            <PrimaryButton onClick={() => handleStart("exam", customMin)} style={{ width: "100%", justifyContent: "center", height: "52px", fontSize: "1rem" }}>
              Start
            </PrimaryButton>
...
function GlobalNav({ theme, onSetTheme, accent, onSetAccent, bgStyle, onSetBgStyle, sets, history, questionSnapshots, onClearAll, screen, profileName, profileIconId, profileBg, profileIColor, onSaveProfile, onRequestClear, sidebarMode = false, forceMobile = false, onToggleForceMobile, onSmartImport, activeSet, allTags, onRenameActiveSet, onSetActiveSetTags, onSetActiveSetIcon, onDeleteActiveSet, reviewResults, reviewQs, historySession, onRequestRetry, onRetryMissed, onRequestDeleteResults, onStudyFromHistory, editCanSave = false, inSearch = false, onDismissSearch }) {
  ...
  function exportActiveSetFn() {
    if (!activeSet) return;
    exportSetAsFile(activeSet);
  }

  function exportResultsFn() {
    const sess = isHist
      ? (historySession ? { ...historySession, questions: resolveSessionQuestions(historySession, questionSnapshots) } : null)
      : (reviewResults && reviewQs ? { results: reviewResults, questions: reviewQs, setName: activeSet?.name, date: new Date().toISOString() } : null);
    if (!sess) return;
    // A detached <a> pointed at a data: URI can get blocked or throw as a top-level navigation
    // on iOS Safari — and since that error was uncaught, it also skipped the close() call right
    // after it, leaving the menu stuck and breaking every export attempted afterward too. The
    // Blob + object-URL approach (same as exportAll()) avoids that entirely.
        ...
        <div style={sidebarMode ? { paddingBottom: "1rem" } : { ...menuPopupStyle({ position: "absolute", right: 0, top: "calc(100% + 8px)", zIndex: 500, minWidth: (inEdit || inResults) ? "180px" : "240px" }) }} className={sidebarMode ? "" : "menu-open"}>
          ...
          {section === null && !inEdit && !inResults && (
            <>
              ...
              <button onClick={() => { exportAll({ sessions: history, snapshots: questionSnapshots }, "pounce-history.json"); close(); }}
                {...surfacePress()} style={{ width: "100%", background: "transparent", border: "none", padding: "0.9rem 1.25rem", fontFamily: FF_SANS, fontSize: "0.95rem", color: T.text, cursor: "pointer", display: "flex", alignItems: "center", gap: "0.75rem" }}>
...
function SetCard({ s, allTags, onEdit, onExport, onStudy, onDelete, onSetTags, onRename, onSetIcon, lastSession, mastery, unanswered, pinned = false, onTogglePin, showSidebar = true, cardColumns = 1 }) {
  const canStudy = s.questions.length > 0;
  // Slightly larger card content specifically for the desktop 3-column grid — the column count
  // itself stays at 3, so this is purely about giving each card a bit more visual presence.
  const isDesktopCard = cardColumns === 3;
  const iconBoxSize  = isDesktopCard ? "76px"   : "64px";
  const iconSize     = isDesktopCard ? 48       : 42;
  const iconSizeFB   = isDesktopCard ? 40       : 34; // fallback (no icon chosen) paw glyph
  const titleSize    = isDesktopCard ? "1.02rem" : "0.95rem";
  const titleMinH    = isDesktopCard ? "2.86rem" : "2.66rem";
  const subtitleSize = isDesktopCard ? "0.76rem" : "0.72rem";
  const rowGap       = isDesktopCard ? "1.05rem" : "0.85rem";
  ...
  // Closes on any pointerdown outside the menu — capture phase so it fires (and closes this
  // menu) before the same click/right-click/long-press reaches its actual target, so a new
  // context menu can open on a different card in the same gesture. The previous approach (a
  // full-viewport invisible div sitting on top to catch the "outside" click) blocked that click
  // from ever reaching the new target at all, silently swallowing the gesture instead of
  // switching menus.
  useEffect(() => {
    if (!ctxMenu) return;
    ...
    <AppCard cardRef={cardRef} onClick={() => canStudy && onStudy(s)}
      ...
      {ctxMenu && onTogglePin && ReactDOM.createPortal(
        ...
        <div onClick={e => e.stopPropagation()}>
          <div ref={ctxMenuRef} className="menu-open-tl" style={{ ...menuPopupStyle({ position: "fixed", top: ctxMenu.top, left: ctxMenu.left, zIndex: 9999, minWidth: "200px" }) }}>
            ...
            {onExport && (
              <KebabMenuItem onClick={() => { closeCtxMenu(); exportSetAsFile(s); }}>
      ...
      <div style={{ display: "flex", gap: rowGap }}>
        ...
        <div style={{
          ...
          {iconDef ? (
            <svg width={iconSize} height={iconSize} viewBox="0 0 24 24" fill="none" stroke={iconStroke} strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
              <path d={iconDef.path} />
            </svg>
          ) : (
            <svg width={iconSizeFB} height={iconSizeFB} viewBox="0 0 100 100" fill="none" stroke={iconStroke} strokeWidth="5">
              <ellipse cx="50" cy="68" rx="22" ry="18"/>
              <ellipse cx="22" cy="46" rx="11" ry="13" transform="rotate(-15 22 46)"/>
              <ellipse cx="42" cy="32" rx="11" ry="13" transform="rotate(-5 42 32)"/>
              <ellipse cx="63" cy="32" rx="11" ry="13" transform="rotate(5 63 32)"/>
        ...
        <div style={{ flex: 1, minWidth: 0, display: "flex", flexDirection: "column", minHeight: iconBoxSize }}>
          <p style={{ fontFamily: FF_SANS, fontWeight: 600, color: canStudy ? T.text : T.muted, fontSize: titleSize, lineHeight: 1.4, margin: 0,
            display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical", overflow: "hidden", minHeight: titleMinH }}>
            {s.name}
          </p>
          <span style={{ fontSize: subtitleSize, fontFamily: FF_SANS, letterSpacing: "0.05em", color: T.muted, marginTop: "0.2rem" }}>
            ...
            <div className="no-scrollbar" style={SET_CARD_SCROLL_GRID}>
              {tagSets.map(s => { const info = setInfo(s); return (
                <SetCard key={s.id} s={s} allTags={allTags}
                  ...
                  pinned={pinnedSetIds.includes(s.id)} onTogglePin={onTogglePin} showSidebar={showSidebar} cardColumns={cardColumns} />
              ); })}
...
function SetsTab({ sets, allTags, untaggedSets, onEdit, onExport, onStudy, onDelete, onSetTags, onSetIcon, onRename, externalSearch, externalActiveTag, externalFilterOpen, onSetFilterOpen, onSetActiveTag, cardColumns = 1, onCreate, history = [], pinnedSetIds = [], onTogglePin, showSidebar = true }) {
        ...
        <>
          {allTags.map(tag => (
            <TagSection key={tag} tag={tag} sets={sets} allTags={allTags}
              ...
              pinnedSetIds={pinnedSetIds} onTogglePin={onTogglePin} showSidebar={showSidebar} />
          ))}
...
function SearchScreen({ sets, history, questionSnapshots, allTags, onEdit, onStudy, onViewHistory, onDelete, onDeleteHistory, onSetTags, onSetIcon, onRename, onExport, inputRef: externalRef, query: externalQuery, cardColumns = 1, pinnedSetIds = [], onTogglePin, showSidebar = true }) {
  ...
  // Sessions in history only carry a qHash reference now — hydrate the real question content
  // back in before handing the session off to the export modal.
  const handleExportSession = s => setExportSession({ ...s, questions: resolveSessionQuestions(s, questionSnapshots) });
    ...
    <div>
        ...
        <div>
          ...
          {recentHistory.length > 0 && (
            <div>
              ...
              <div style={{ display: "grid", gridTemplateColumns: `repeat(${cardColumns}, 1fr)`, gap: cardColumns === 1 ? "0.5rem" : "1rem" }}>
                {recentHistory.map(h => <HistoryCard key={h.id} session={h} onView={onViewHistory} onStudy={onStudy} matchedSet={sets.find(s => s.id === h.setId || s.name === h.setName)} onExport={handleExportSession} onRequestDelete={setConfirmDel} />)}
              </div>
            </div>
          )}
      ...
      {matchedHistory.length > 0 && (
        <div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: `repeat(${cardColumns}, 1fr)`, gap: cardColumns === 1 ? "0.5rem" : "1rem" }}>
            {matchedHistory.map(h => <HistoryCard key={h.id} session={h} onView={onViewHistory} onStudy={onStudy} matchedSet={sets.find(s => s.id === h.setId || s.name === h.setName)} onExport={handleExportSession} onRequestDelete={setConfirmDel} />)}
          </div>
        </div>
      )}
    </div>
      ...
      <div ref={trackRef} style={{
        ...
        <div ref={panelRefs[0]} style={{ width: `${100 / TAB_ORDER.length}%`, flexShrink: 0, padding: "0 1.25rem 1.5rem" }}>
          <div style={{ marginTop: "11px" }}>
            <Dashboard history={history} sets={sets} questionSnapshots={questionSnapshots} onStudy={onStudy} onViewHistory={onViewHistory} />
          </div>
        </div>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: "0.9rem" }}>
        ...
        <div style={{ flex: 1, minWidth: 0, display: "flex", flexDirection: "column" }}>
          <p style={{ fontFamily: FF_SANS, fontWeight: 600, color: T.text, fontSize: "0.95rem",
            overflow: "hidden", display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical", lineHeight: 1.4, margin: 0 }}>
            {session.setName}
          </p>
...
// ── Compact history row — used inside a day's results widget ──────────────
function HistoryDayRow({ session, onView, onStudy, matchedSet, onExport, onRequestDelete, isLast }) {
  const pct    = Math.round((session.score / session.total) * 100);
  const sColor = scoreColor(pct);
  const canStudy = matchedSet && matchedSet.questions.length > 0;
  ...
  return (
    <div ref={rowRef} onClick={() => onView(session)}
      onContextMenu={e => { e.preventDefault(); setCtxMenu({ top: e.clientY, left: e.clientX }); }}
      ...
      style={{
        ...
        cursor: "pointer",
        background: hover ? (T.mode === "light" ? "rgba(0,0,0,0.02)" : "rgba(255,255,255,0.03)") : "transparent",
        transition: "background 0.15s",
        WebkitTapHighlightColor: "transparent",
      }}>
      {ctxMenu && ReactDOM.createPortal(
        // React portals still bubble synthetic events through the component tree, not the DOM
        // tree — without this stopPropagation, clicks here would also reach this row's onClick
        // (nested above in the React tree) and trigger onView underneath the menu.
        <div onClick={e => e.stopPropagation()}>
          <div style={{ position: "fixed", inset: 0, zIndex: 9998 }}
            onClick={closeCtxMenu}
      ...
      <div style={{ width: "40px", height: "40px", borderRadius: "50%", flexShrink: 0,
        ...
        background: sColor + "1a",
        display: "flex", alignItems: "center", justifyContent: "center" }}>
        <span style={{ fontFamily: FF_SANS, fontWeight: 700, fontSize: "0.66rem",
          color: sColor }}>
          {pct}%
        </span>
      </div>
      <div style={{ flex: 1, minWidth: 0, display: "flex", flexDirection: "column" }}>
        <p style={{ fontFamily: FF_SANS, fontWeight: 600, color: T.text, fontSize: "0.9rem",
          overflow: "hidden", display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical", lineHeight: 1.35, margin: 0 }}>
          {session.setName}
        </p>
        <div style={{ display: "flex", alignItems: "center", gap: "0.5rem", marginTop: "0.25rem" }}>
          <span style={{ fontSize: "0.68rem", fontFamily: FF_SANS, letterSpacing: "0.03em", color: T.muted }}>
            {new Date(session.date).toLocaleTimeString(undefined, { hour: "numeric", minute: "2-digit" })} · {session.score}/{session.total}
          </span>
          {session.mode && (
            <Tag
              label={session.mode === "quick" ? "QUICK " + session.total : session.mode === "exam" ? "EXAM" : "REVIEW"}
...
function formatHistoryDayHeading(date) {
  const now = new Date();
...
function ResultsHistoryView({ history, sets = [], questionSnapshots, onImport, onDelete, onView, onStudy, externalSearch, externalSortBy }) {
  const [exportSession, setExportSession] = useState(null);
  const [confirmDel,    setConfirmDel]    = useState(null);
  const [expandedDays,  setExpandedDays]  = useState({}); // { [dayKey]: true }
  ...
  // Group into day sections, preserving each session's place within its day
  // according to the active sort. Day sections themselves are ordered newest
  // first, unless the sort is explicitly oldest-first.
  const dayGroups = [];
  const dayLookup = {};
  sorted.forEach(session => {
    ...
    if (!(key in dayLookup)) {
      dayLookup[key] = dayGroups.length;
      dayGroups.push({ key, date: d, sessions: [] });
    }
    dayGroups[dayLookup[key]].sessions.push(session);
  });
  dayGroups.sort((a, b) => sortBy === "date-asc" ? a.date - b.date : b.date - a.date);
    ...
    <div>
      ...
      {dayGroups.length > 0 && (
        <div style={{ display: "flex", flexDirection: "column", gap: "1.75rem" }}>
          {dayGroups.map(group => {
            const isExpanded = !!expandedDays[group.key];
            const visible = isExpanded ? group.sessions : group.sessions.slice(0, HISTORY_DAY_PAGE_SIZE);
            const hasMore = group.sessions.length > HISTORY_DAY_PAGE_SIZE;
            return (
              <div key={group.key}>
                <p style={{ fontFamily: FF_SANS, fontSize: "1.2rem", fontWeight: 700, color: T.text, marginBottom: "0.75rem" }}>
                ...
                <div style={card({ padding: 0, overflow: "hidden" })}>
                  ...
                  {hasMore && (
                    <button onClick={() => setExpandedDays(e => ({ ...e, [group.key]: !e[group.key] }))}
                      style={{
                      ...
                      {isExpanded ? "Show less" : "Show all " + group.sessions.length + " results"}
                    </button>
                  )}
                </div>
              </div>
...
function Dashboard({ history, sets, questionSnapshots, onStudy, onViewHistory }) {
  const canHover = window.matchMedia("(hover: hover)").matches;
  const totalSessions  = history.length;
  const totalQuestions = history.reduce((sum, s) => sum + s.total, 0);
  const mastery        = computeMastery(history, sets.flatMap(s => s.questions));
  ...
  const lastColor = scoreColor(Math.round(lastPct * 100));
  const lastPassed = lastColor === T.green;

  // Per-question topic accuracy — keyed by qId so quick-quiz repetition doesn't skew counts
  const qTopicMap = {};
  history.forEach(function(session) {
    if (!session.results) return;
    const sessionQuestions = resolveSessionQuestions(session, questionSnapshots);
    if (!sessionQuestions.length) return;
    session.results.forEach(function(r) {
      const q = sessionQuestions.find(function(q) { return q.id === r.qId; });
      if (!q) return;
      const topic = q.topic && q.topic.trim() ? q.topic.trim() : null;
      if (!topic) return;
      ...
      {lastSession && (
        <div className="card-fade-up" style={{ animationDelay: "150ms" }}>
          ...
          <div
            ...
            <div style={{ display: "flex", alignItems: "center", gap: "1.1rem", padding: "0.8rem 1.25rem" }}>
              <div style={{ width: "68px", height: "68px", borderRadius: "50%", flexShrink: 0,
                border: "3px solid " + lastColor,
                background: lastColor + "1a",
                display: "flex", alignItems: "center", justifyContent: "center" }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <p style={{ fontFamily: FF_SANS, fontWeight: 700, fontSize: "1rem",
                  color: lastColor, marginBottom: "0.2rem" }}>
...
function FloatingHomeBar({ homeTab, setHomeTab, historyBadgeCount = 0, disabled, onSetsTab, onCreate, onImport }) {
  const [fabOpen, setFabOpen] = useState(false);
  const [fabClosing, setFabClosing] = useState(false);
  const [fabMenuPos, setFabMenuPos] = useState(null);
  ...
  const tabs = [
    { id: "home",    label: "Home",
      ...
      svgFilled: <svg width="22" height="22" viewBox="0 0 24 24" fill="currentColor" stroke="none"><path d="M3 10a2 2 0 0 1 .709-1.528l7-5.999a2 2 0 0 1 2.582 0l7 5.999A2 2 0 0 1 21 10v9a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"/></svg> },
    { id: "sets",    label: "Sets",
      ...
      svgFilled: <svg width="22" height="22" viewBox="0 0 24 24" fill="currentColor" stroke="none"><rect x="3" y="3" width="7" height="7" rx="1.5"/><rect x="14" y="3" width="7" height="7" rx="1.5"/><rect x="3" y="14" width="7" height="7" rx="1.5"/><rect x="14" y="14" width="7" height="7" rx="1.5"/></svg> },
    { id: "history", label: "History",
      badge: historyBadgeCount > 0 ? historyBadgeCount : null,
...
function App() {
  ...
  const [questionSnapshots, setQuestionSnapshots] = useState(() => loadQuestionSnapshots());
  ...
  // Number of history sessions that existed the last time the History page was actually
  // viewed. The badge shows how many sessions have been added since then, not the raw total.
  const [historySeenCount, setHistorySeenCount] = useState(() => {
    const v = parseInt(localStorage.getItem(HISTORY_SEEN_KEY), 10);
    return Number.isFinite(v) ? v : 0;
  });
  useEffect(() => { localStorage.setItem(HISTORY_SEEN_KEY, String(historySeenCount)); }, [historySeenCount]);
  const historyBadgeCount = Math.max(0, (history?.length || 0) - historySeenCount);
  // Always land at the top of the page on any screen transition — home ⇄ edit ⇄ review ⇄
  // results/historyResults. Without this, a screen inherits whatever scroll position the
  // previous screen was left at (e.g. scrolling far down in the question builder, then
  // backing out to Sets, would leave Sets scrolled to that same spot).
  React.useLayoutEffect(() => {
    window.scrollTo(0, 0);
    document.documentElement.scrollTop = 0;
    document.body.scrollTop = 0;
  ...
  // Clear the "new since last viewed" history badge whenever the History tab is actually
  // on screen. Re-runs if more sessions arrive while already viewing it, keeping it at 0.
  useEffect(() => {
    if (screen === "home" && homeTab === "history") setHistorySeenCount(history.length);
  }, [screen, homeTab, history.length]);
  ...
  useEffect(() => { saveQuestionSnapshots(questionSnapshots); }, [questionSnapshots]);
  ...
  function handleRename(setId, name) {
    setSets(prev => prev.map(s => s.id === setId ? { ...s, name } : s));
    // History sessions store setName as a snapshot from when they were saved — without this,
    // every session recorded before the rename would keep showing the old name forever, and
    // searching history by the new name wouldn't find them.
    setHistory(prev => prev.map(h => h.setId === setId ? { ...h, setName: name } : h));
  }
  ...
  function completeClearAll() {
    ...
    localStorage.removeItem(HISTORY_SNAPSHOTS_KEY);
  ...
  function handleImport(imported) {
    ...
    // Sets that already exist here (matched by id) are never duplicated or overwritten — except
    // for their name. Re-importing a set you renamed on another device is the only way that
    // rename can reach this one, so it needs to actually take effect here rather than being
    // silently skipped as "already imported". Everything else (questions, tags, icon) is left
    // untouched, in case this device has its own independent edits to that same set.
    const renamedIds = []; // [{ setId, name }] — only where the imported name actually differs
    setSets(prev => {
      const byId = new Map(normalized.map(s => [s.id, s]));
      const merged = prev.map(s => {
        const incoming = byId.get(s.id);
        if (incoming && incoming.name !== s.name) {
          renamedIds.push({ setId: s.id, name: incoming.name });
          return { ...s, name: incoming.name };
        }
        return s;
      });
      const existingIds = new Set(prev.map(s => s.id));
      return [...merged, ...normalized.filter(s => !existingIds.has(s.id))];
    });
    // Keep this device's own history sessions in sync with the newly-synced name, same as a
    // local rename would — otherwise the History tab would keep showing the pre-rename name
    // for every session already recorded here.
    if (renamedIds.length) {
      setHistory(prev => prev.map(h => {
        const r = renamedIds.find(r => r.setId === h.setId);
        return r ? { ...h, setName: r.name } : h;
      }));
    }
  }
  ...
  function handleSaveToHistory(session) {
    const { session: deduped, hash, snapshot } = dedupeSessionQuestions(session);
    if (hash && !(hash in questionSnapshots)) {
      setQuestionSnapshots(prev => (hash in prev) ? prev : { ...prev, [hash]: snapshot });
    }
    setHistory(prev => {
      const exists = prev.find(s => s.id === deduped.id);
      if (exists) return prev; // no duplicates
      return [...prev, deduped];
    });
    setLastSavedSessionId(deduped.id);
  }

  function handleImportHistory(sessions, incomingSnapshots = {}) {
    // Merge any snapshots that came along with an exported bundle first (existing entries win
    // on hash collision, though collisions are effectively impossible with matching content).
    if (incomingSnapshots && Object.keys(incomingSnapshots).length) {
      setQuestionSnapshots(prev => ({ ...incomingSnapshots, ...prev }));
    }
    // Legacy sessions (pre-dedup exports) still carry a full embedded `questions` array —
    // dedupe those on the way in too, so re-importing an old export doesn't reintroduce bloat.
    const newSnapshots = {};
    const normalized = sessions.map(s => {
      if (!s.questions) return s;
      const { session: deduped, hash, snapshot } = dedupeSessionQuestions(s);
      if (hash) newSnapshots[hash] = snapshot;
      return deduped;
    });
    if (Object.keys(newSnapshots).length) {
      setQuestionSnapshots(prev => ({ ...newSnapshots, ...prev }));
    }
    setHistory(prev => {
      const existingIds = new Set(prev.map(s => s.id));
  ...
  function handleSmartImport(f) {
    ...
    r.onload = ev => {
      try {
        const parsed = JSON.parse(ev.target.result);
        // Dedup-bundle export format: { sessions: [...], snapshots: { hash: questions } }
        if (parsed && !Array.isArray(parsed) && Array.isArray(parsed.sessions)) {
          const v = parsed.sessions.filter(validateSession);
          if (v.length) {
            handleImportHistory(v, parsed.snapshots || {});
            showToast(`Loaded ${v.length} session${v.length !== 1 ? "s" : ""}`);
  ...
  function handleDeleteHistory(id) {
    setHistory(prev => {
      ...
      // If no other session still references this snapshot, it's dead weight — drop it.
      if (target?.qHash && !next.some(s => s.qHash === target.qHash)) {
        setQuestionSnapshots(snaps => {
          const { [target.qHash]: _, ...rest } = snaps;
          return rest;
        });
      }
      return next;
    ...
    <div style={{ background: bgStyle === "gradient"
      ...
      <div style={{ minHeight: "100vh", visibility: (showDeleteAnim || showWelcome) ? "hidden" : "visible",
        ...
        <div style={{
          ...
          {/* Keyed on modalOpen so it remounts the instant a full-screen Modal closes, forcing
              the browser to build a fresh backdrop-filter layer immediately (via the
              sf-blur-fix trick) instead of slowly re-establishing the stale one underneath
              the modal's own backdrop-filter — that re-establishment is what caused the
              visible ~0.5s pop-in delay. */}
          <div key={String(modalOpen)} style={{
          ...
          <div style={{ width: "100%", maxWidth: showSidebar ? "1200px" : (isDesktop || isTablet) ? "900px" : "720px", height: showSidebar ? "48px" : undefined, padding: showSidebar ? "0 1rem" : editingSetName && !(screen === "edit" && (isDesktop || isTablet || titleBarVisible)) ? "0.5rem 1rem 1.75rem" : "0.5rem 1rem", display: "flex", position: "relative", zIndex: 1 }}>
            ...
            {(!showSidebar || screen !== "home") && <GlobalNav theme={theme} onSetTheme={handleSetTheme} accent={accent} onSetAccent={handleSetAccent} bgStyle={bgStyle} onSetBgStyle={handleSetBgStyle}
              sets={sets} history={history} questionSnapshots={questionSnapshots} onClearAll={handleClearAll} screen={screen}
        ...
        <div style={{ position: "relative", display: "flex", justifyContent: "center", flex: 1,
          padding: screen === "home" ? (showSidebar ? "1.5rem 0 4rem" : "1.5rem 0 7rem") : (screen === "review" || screen === "edit") ? "1.5rem 0 7rem" : "calc(1.5rem + 6px) 0 4rem",
        }}>
          <div style={{ width: "100%", maxWidth: showSidebar ? "1200px" : (isDesktop || isTablet) ? "900px" : "720px", padding: "0 1rem" }}>
            ...
            {screen === "historyResults" && historySession && (
              <ResultsScreen
                ...
                questions={resolveSessionQuestions(historySession, questionSnapshots)}
      ...
      {showSidebar && (
        <div style={{
          ...
          {/* Background/blur split into its own layer, keyed on modalOpen so it remounts the
              instant a full-screen Modal closes — forces the browser to build a fresh
              backdrop-filter layer immediately (via the sf-blur-fix trick) instead of slowly
              re-establishing the stale one underneath the modal's own backdrop-filter, which
              caused a visible ~0.5s pop-in delay. Split out (rather than keying the whole
              sidebar) so remounting it doesn't reset scroll position or nested popup state. */}
          <div key={String(modalOpen)} style={{
            position: "absolute", inset: 0, zIndex: -1, pointerEvents: "none", borderRadius: "16px",
              ...
              return (
                <button key={tab.id} onClick={() => { setHomeTab(tab.id); setScreen("home"); if (tab.id !== "search") setSearchQuery(""); if (tab.id === "sets") setSetsSearch(""); }}
                  ...
                  {!sidebarCollapsed && (
                    <>
                      {tab.label}
                      {tab.id === "history" && historyBadgeCount > 0 && (
                        <span style={{ marginLeft: "auto", background: ST.accent + "22", color: ST.accent, fontFamily: FF_SANS, fontSize: "0.65rem", fontWeight: 600, padding: "0.1rem 0.4rem", borderRadius: "99px" }}>{historyBadgeCount}</span>
                      )}
                    </>
                  )}
                </button>
          ...
          {sidebarSection === null && (
            <>
              ...
              <div style={{ padding: "0.25rem 0.5rem" }}>
                ...
                <SidebarActionButton onClick={() => exportAll({ sessions: history, snapshots: questionSnapshots }, "pounce-history.json")} icon={<span style={{ fontSize: "0.9rem" }}>◷</span>} label="Save all history" />
